...
>
> Status consolidado em 02/dez/2025.
...
- **Resumo do estado atual (02/dez/2025)**
  ...
  - **Correção crítica aplicada (02/dez/2025)**: Pipeline de votos corrigido - `votos_deputados` agora possui 40.855 registros válidos (id_deputado > 0) distribuídos em 2022-2025. Migration 018 aplicada para remover índice único problemático em `id_votacao_camara`. Rankings de presença funcionando corretamente no frontend.
  - Em andamento: validação de performance em staging e cobertura de repositórios sem integração automatizada.
  - Pontos de atenção: proposições com apenas 2 registros em cache, sub-recursos de deputados (discursos, eventos, histórico, etc.), filtros avançados de proposições (arrays, `codTema`, `autor`).
  - Próximos alvos (prioridade média): Órgãos, Legislaturas, Referências, acelerar backfill de proposições.
...
- [ ] Ajustar pipeline de votos para persistir `id_deputado` correto (atualmente gravando `0`, o que zera ranking de presença) *(✅ CORRIGIDO em 02/dez/2025 - migration 018 aplicada, backfill executado com 40.855 votos válidos)*
...
**Próximos passos imediatos (02/dez/2025)**
1. ~~Corrigir pipeline de votos para persistir `id_deputado` oficial~~ ✅ CONCLUÍDO - 40.855 votos válidos no banco
2. ~~Reexecutar `POST /api/v1/analytics/rankings/atualizar`~~ ✅ CONCLUÍDO - Rankings funcionando no frontend
3. Acelerar o backfill de proposições: destravar ingestão no ingestor (batches anuais + checkpoints) e popular `proposicoes_cache` com autores e metadados completos. *(Atualmente apenas 2 registros)*
...
5. Documentar para SRE o estado atual do backfill (despesas concluído, proposições pendentes) e atualizar runbook de monitoramento.
...
8. Popular tabela `partidos` (atualmente vazia).
